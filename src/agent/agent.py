...
def collect_real_data(connection, psutil):
    ...
    if psutil:
        # Prime cpu_percent calls for all processes
        ...
        all_procs = []
        attrs = ['pid', 'name', 'username', 'cpu_percent', 'memory_info', 'io_counters']
        for p in psutil.process_iter(attrs):
            try:
                p_info = p.info
                # Create a clean, serializable dictionary with the exact keys the frontend expects
                clean_proc = {
                    'pid': p_info.get('pid'),
                    'name': p_info.get('name'),
                    'username': p_info.get('username'),
                    'cpu_usage': p_info.get('cpu_percent'), # Key is cpu_usage
                    'memory_mb': 0, # In MB
                    'read_mb': 0,   # In MB
                    'write_mb': 0,  # In MB
                }
                ...
                if mem_info:
                    clean_proc['memory_mb'] = round(mem_info.rss / (1024 * 1024), 2)
                
                io = p_info.get('io_counters')
                if io:
                    clean_proc['read_mb'] = round(io.read_bytes / (1024 * 1024), 2)
                    clean_proc['write_mb'] = round(io.write_bytes / (1024 * 1024), 2)
        ...
        top_cpu_processes = sorted([p for p in all_procs if p.get('cpu_usage') is not None], key=lambda p: p['cpu_usage'], reverse=True)[:10]
        top_memory_processes = sorted([p for p in all_procs if p.get('memory_mb') is not None], key=lambda p: p['memory_mb'], reverse=True)[:10]
        top_io_processes = sorted(all_procs, key=lambda p: p['read_mb'] + p['write_mb'], reverse=True)[:10]
...
def send_data(data):
    ...
    try:
        ...
        # Add a debug log to confirm counts
        print(f"Process counts sent: CPU={len(data.get('topCpuProcesses', []))}, Memory={len(data.get('topMemoryProcesses', []))}, IO={len(data.get('topIoProcesses', []))}")
