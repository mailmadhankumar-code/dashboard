...
import { Database, Building, Activity, AlertTriangle, ShieldAlert, LogOut, LayoutGrid } from "lucide-react";
...
export default function DashboardSidebar({
    ...
    alerts.forEach(alert => {
        ...
        if (!alertsByCustomer.has(customerName)) {
            alertsByCustomer.set(customerName, []);
        }
        alertsByCustomer.get(customerName)!.push(alert);
    });

    const customersWithAlerts = customers
        .map(customer => ({
            ...customer,
            customerAlerts: alertsByCustomer.get(customer.name) || []
        }))
        .filter(customer => customer.customerAlerts.length > 0);
    ...
    return (
        <>
            ...
            <SidebarContent>
                <SidebarMenu>
                    <SidebarMenuItem>
                        <Link href="/overview" passHref>
                            <SidebarMenuButton isActive={pathname === '/overview'}>
                ...
                <SidebarSeparator />
                <Accordion type="multiple" defaultValue={[...customers.map(c => c.id), 'alerts']} className="w-full">
                    ...
                    <AccordionItem value="alerts" className="border-none">
                        ...
                        <AccordionContent className="pt-2 text-sidebar-foreground">
                            {customersWithAlerts.length === 0 ? (
                                ...
                                <Accordion type="multiple" defaultValue={customersWithAlerts.map(c => c.id)} className="w-full">
                                    {customersWithAlerts.map(customer => (
                                        <AccordionItem value={customer.id} key={customer.id} className="border-none">
                                            <AccordionTrigger className="px-2 hover:no-underline hover:bg-sidebar-accent rounded-md text-sm justify-start gap-2">
                                                <Building className="w-4 h-4" />
                                                <span>{customer.name}</span>
                                                <Badge variant="destructive" className="ml-auto">{customer.customerAlerts.length}</Badge>
                                            </AccordionTrigger>
                                            <AccordionContent className="pt-2">
                                                <SidebarMenu>
